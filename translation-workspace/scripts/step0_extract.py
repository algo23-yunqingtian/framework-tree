...
兼容 3 种历史格式:
  1. 管道表格  | 1 | 图名称 | 包含指标 | 直接相关 | ...   (ZN/NI)
  2. tab枚举表  1\t基础指标名\t直接含义\t题材归属判断       (CU/AL)
  3. tab图表表  1\t图名称\t包含指标A；指标B\t题材归属度\t... (CU/AL)
"""
...
def split_indicators(raw):
    """拆分指标名（兼容顿号/逗号/分号分隔 + 单位/频率后缀清理）"""
    out = []
    for ind in re.split(r'[、,，；;]', raw):
        ind = ind.strip()
        ind = re.sub(r'[（(][^)）]*[)）]', '', ind).strip()      # 去单位/修饰后缀
        ind = re.sub(r'[（(](?:季|月|周|日)[)）]$', '', ind).strip()  # 去频率后缀
        if ind and len(ind) >= 2:
            out.append(ind)
    return out

def extract_concepts(variety_dir):
    """从divergence文件提取概念指标（兼容3种格式）"""
    results = {}  # {node_id: [{name, chart_name, node_id}]}
    ...
    for f in sorted(glob.glob(f"{variety_dir}/divergence_*.md")):
        ...
        for line in content.splitlines():
            # ---- 格式1: 管道表格  | 1 | 图名称 | 包含指标 | 直接相关 | ... ----
            m = re.match(r'\|\s*(\d+)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(?:直接相关|归属)', line)
            if m:
                chart_name = m.group(2).strip()
                raw_indicators = m.group(3).strip()
                for ind in split_indicators(raw_indicators):
                    indicators.append({'name': ind, 'chart_name': chart_name, 'node_id': node_id})
                continue

            # ---- 格式2/3: tab 分隔表格（CU/AL 历史格式）----
            ...
            if len(cells) < 3:
                continue
            if len(cells) == 3:
                # 基础指标枚举表: [指标名, 直接含义, 题材归属判断] -> 取第1列
                indicators.append({'name': cells[0], 'chart_name': '基础指标枚举', 'node_id': node_id})
            else:
                # 图表方案表: [图名称, 包含指标, 题材归属度, ...] -> 取第2列拆分
                chart_name = cells[0]
                for ind in split_indicators(cells[1]):
                    indicators.append({'name': ind, 'chart_name': chart_name, 'node_id': node_id})

...
def build_prompt(task):
    ...
    # 疑似海关/关税/国别贸易主题触发内容风控。加投资研究范畴声明 + 弱化指令语气。
    t = body.replace("{品种}", VAR_CN[v]).replace("{板块名}", dim_name).replace("{N}", str(n))
    if dim_name == "进出口":
        t = ("本任务是商品期货研究中的图表指标梳理，属于常规投资研究内容，请正常协助。\n\n") + t
        t = t.replace("请你做以下5项工作", "请协助核对以下5个方面")
        t = t.replace("【任务1：删除不合格指标】严格删除以下类型", "【第1方面：识别不合适的指标】请指出以下类型指标并说明原因")
        t = t.replace("删除后如有空位", "识别后如出现空缺")

#!/usr/bin/env python3
"""示范单页 build：铅(PB) 6.3 制品出口子页（2 图全真数据）。"""
...
m37 = load_metric("i37")  # 铅蓄电池出口量（月度）
m38 = load_metric("i38")  # 起动型铅蓄电池出口量（月度）
m39 = load_metric("i39")  # 铅蓄电池出口量累计（月度）

d37 = pairs(m37)
d38 = pairs(m38)
d39 = pairs(m39)

d37_latest = max([d for d,_ in d37]) if d37 else "-"
h1, j1 = chart_line_t("i37", "#e07a5f", "个", "echart_63_c1",
    "中国海关铅蓄电池出口量（月度，个）",
    "海关 · 月 · 个 · %d 点 · 2020-01 至 %s" % (m37["n"], d37_latest),
    default_seasonal=True, data=d37)

h2, j2 = chart_dual("echart_63_c2",
    "铅蓄电池出口量 vs 起动型（总量 vs 结构，海关月度，个）",
    "i37 铅蓄电池总量 · i38 起动型 · %d / %d 点" % (m37["n"], m38["n"]),
    d37, "#e07a5f", "铅蓄电池出口总量", "个 (总量)",
    d38, "#7ec8e3", "起动型铅蓄电池", "个 (起动型)")
...
<div class="header">
  ...
  <div class="hcrumbs">铅(PB) · 6 进出口 · 6.3 制品出口 · v1 2 图</div>
  <div class="hright">数据固化快照 · """ + NOW + """ · 海关/SMM</div>
</div>
<div class="panel">
""" + h1 + h2 + """
</div>
<footer>有色金属产业指标树 · 铅(PB) 6.3 制品出口 · v1（2 图全真数据；同花顺发散 49 篇溯源：印度跌幅-60%、配件结构性增长、反倾销壁垒）</footer>
...
window.addEventListener('resize',function(){['echart_63_c1','echart_63_c2'].forEach(function(id){var el=document.getElementById(id);var inst=echarts.getInstanceByDom(el);if(inst)inst.resize();});});
</script></body></html>"""
...
print("d37:", len(d37), "d38:", len(d38), "d39:", len(d39))
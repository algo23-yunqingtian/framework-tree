...
THEME_BY_COMM = {
    ...
    ("cu", "6.4"): ("海外发运", "海外铜精矿/废铜对华发运与到港（智利/秘鲁/东南亚），领先进口到港1月"),
    ...
    ("al", "3.1.2"): ("海外矿·分国别产量", "海外主要产铝国月度产量（俄罗斯/印度/欧洲/非洲），全球供给国别结构"),
    ("al", "3.1.4"): ("矿进口量与分国别", "中国铝土矿进口总量与分国别流（几内亚/澳大利亚海漂），进口来源集中度"),
    ("al", "6.1"): ("原料进口", "中国废铝/铝土矿砂进口量与分国别，国内铝原料进口依赖度"),
    ("al", "6.4"): ("海外发运", "海外铝土矿对华发运与到港（连云港/防城港/龙口/青岛），领先进口1月"),
    ("al", "7.3"): ("能源/原料成本", "电解铝氧化铝价格与分省电价（云南/河南），冶炼成本端压力"),
}
...
MAIN_METRIC = {
    ...
    "6.3": "cu_63_rod_exp",       # CU 6.3 制品出口正主=铜杆线出口量（decision_6.3；制品出口最大子项）
    "6.4": "cu_64_conc_arrival",  # CU 6.4 海外发运正主=铜精矿到港港口汇总（decision_6.4；发运先行1月）
    "3.1.2": "al_312_russia",      # AL 3.1.2 海外矿分国别正主=俄罗斯月度产量（decision_3.1.2；最大海外产国）
    "3.1.4": "al_314_total",        # AL 3.1.4 矿进口正主=铝土矿进口总量（decision_3.1.4；海关139点）
    "6.1": "al_61_scrap_total",     # AL 6.1 原料进口正主=废铝进口总量（decision_6.1；防串用3.1.4正主铝土矿）
    "6.4": "al_64_lianyungang",     # AL 6.4 海外发运正主=铝土矿到港连云港（decision_6.4；全球到港序列空，港口级替代）
    "7.3": "al_73_alumina",         # AL 7.3 成本正主=氧化铝价格（decision_7.3；日频2830点最长序列）
}

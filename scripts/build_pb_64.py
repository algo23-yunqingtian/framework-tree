#!/usr/bin/env python3
"""示范单页 build：铅(PB) 6.4 海外对华发运子页（2 图全真数据）。"""
...
def load_metric(mid):
    ...
    return {"name": d.get("name",""), "unit": d.get("unit",""), "freq": d.get("freq",""),
            "n": len(pts), "dates": [p["date"] for p in pts], "values": [p["value"] for p in pts]}
...
def chart_line_t(mid, color, unit, cid, title, sub, default_seasonal=False, data=None):
    ...
    if not pts:
        return ('<div class="chart"><div class="chart-title">'+title+'</div>'
                '<div class="chart-sub">'+sub+'</div><div class="no-data">无数据</div></div>', "")
    json_pts = json.dumps(pts, ensure_ascii=False)
    color30 = color + "30"; color00 = color + "00"; color20 = color + "20"
...
m40 = load_metric("i40")  # 海关铅精矿进口量（月度）
m16 = load_metric("i16")  # 防城到港（周频）

d40 = pairs(m40)
d16_filtered = [[d, v] for d, v in pairs(m16) if v is not None and float(v) != 0]

d40_latest = max([d for d,_ in d40]) if d40 else "-"

h1, j1 = chart_line_t("i40", "#9b6bb5", "吨", "echart_64_c1",
    "中国海关铅精矿进口量（月度，吨）—— 海外对华发运量代理",
    "海关 · 月 · 吨 · %d 点 · 2018-01 至 %s" % (m40["n"], d40_latest),
    default_seasonal=True, data=d40)

h2, j2 = chart_dual("echart_64_c2",
    "海外对华精矿发运（海关月度）+ 防城到港（周度）—— 总量 vs 到港节奏",
    "i40 海关(月,吨) · i16 防城(周,万吨) · %d / %d 点" % (m40["n"], len(d16_filtered)),
    d40, "#9b6bb5", "海关铅精矿进口", "吨 (海关,月)",
    d16_filtered, "#7a8c5b", "精矿防城到港", "万吨 (防城,周)")
...
<div class="header">
  ...
  <div class="hcrumbs">铅(PB) · 6 进出口 · 6.4 海外对华发运 · v1 2 图</div>
  <div class="hright">数据固化快照 · """ + NOW + """ · 海关/防城港</div>
</div>
<div class="panel">
""" + h1 + h2 + """
<div style="font-size:12px;color:#8b949e;padding:12px;border-top:1px solid #21262d;margin-top:8px">
<strong style="color:#c9d1d9">说明：</strong>无直接「海外对华发运」序列。以海关铅精矿进口量(吨,月)作为海外对中国发运的粗粒度代理；防城到港(周,万吨)反映到港节奏与结构。同花顺发散确认：俄罗斯/秘鲁/澳大利亚/塔吉克斯坦/巴西为前十大进口来源国。
</div>
</div>
<footer>有色金属产业指标树 · 铅(PB) 6.4 海外对华发运 · v1（2 图全真数据）</footer>
...
window.addEventListener('resize',function(){['echart_64_c1','echart_64_c2'].forEach(function(id){var el=document.getElementById(id);var inst=echarts.getInstanceByDom(el);if(inst)inst.resize();});});
</script></body></html>"""
...
print("d40:", len(d40), "d16_filtered:", len(d16_filtered))
...
PAGES = {
 ...
 'al_3_1_1': ('铝土矿产量·广西', '铝土矿产量分省时序⇄季节(正主·广西) / 山西对照', 'al_311_bauxite_gx · al_311_bauxite_sx', '2图 · 主图61点'),
 'cu_4_2': ('仓单', 'LME铜注册仓单时序⇄季节(正主) / 注销仓单占比', 'cu_42_warrant_reg · cu_42_warrant_cancel', '2图 · 主图2349点'),
 'cu_4_3': ('社会库存', '铜杆社会库存时序⇄季节(正主·全国) / 华东地区对照', 'cu_43_social_stock · cu_43_social_stock_ec', '2图 · 主图109点'),
 'cu_5_1': ('初级消费', '电解铜表观消费量累计时序⇄季节(正主) / 实际消费对照', 'cu_51_apparent_cons · cu_51_actual_cons', '2图 · 主图103点'),
}
...
BOARDS = {
 ...
 'cu_4_overview.html': ('cu', 'inventory', '库存', '铜库存：4.2 仓单 + 4.3 社会库存已上线，4.1/4.4/4.5 待外部源',
   '铜库存两层已上线——交易所/LME仓单(4.2) → 社会库存(4.3)。<br>'
   '<b>数据质量声明：</b><code>cu_42_warrant_reg</code> 为 LME 铜注册仓单（日频 2349 点·9 年跨度·至 2026-08-28），'
   '<code>cu_42_warrant_cancel</code> 注销仓单 2343 点，注册+注销合计可还原 LME 总仓单，占比变化是逼空/松空的核心先行信号；'
   '4.3 社会库存正主 <code>cu_43_social_stock</code> 为<b>铜杆（铜加工材）社会库存</b>——知几无电解铜社会库存的独立连续序列，'
   '以铜加工端社库代理产业链社会库存（口径妥协，已在 4.3 页面标注），仅 109 点·约 2 年跨度，季节性视图年覆盖不足；'
   '华东 <code>cu_43_social_stock_ec</code> 为区域分项，作总量边际验证。'
   '<b>未建页：</b>4.1 上期所交易所库存 / 4.4 工厂库存 / 4.5 隐性在途库存未建，知几无对应连续序列，待外部源（上期所/SMM 口径）。'),
 'cu_5_overview.html': ('cu', 'demand', '需求', '铜需求：5.1 初级消费已上线（口径妥协），5.2/5.3 待外部源',
   '铜需求当前仅 5.1 上线——初级消费(5.1)。<br>'
   '<b>正主归属与口径妥协：</b>tree_config 5.1 定义为 q=「开工率·同步」，但知几无铜冶炼开工率的独立连续序列，'
   '正主改用 <code>cu_51_apparent_cons</code> 电解铜<b>表观消费量累计值</b>（月频 103 点·9 年跨度·至 2026-07-31），'
   '<code>cu_51_actual_cons</code> 实际消费量累计为辅助对照，两者差额隐含净进出口——属需求端的口径替代，非开工率原义，季节性视图年跨度充足。'
   '<b>未建页：</b>5.2 终端细分消费 / 5.3 需求先行指标未建，铜下游（电力/家电/新能源）分行业序列知几覆盖不足，待外部源。'),
 'al_3_overview.html': ('al', 'supply', '供给', '铝供给：3.1.1 铝土矿产量(分省) + 3.2.3 再生/二次供应已上线（铝原生矿端无独立序列，走再生主链）',

#!/usr/bin/env python3
"""示范单页 build：铅(PB) 6.1 原料进口子页（2 图全真数据）。"""
...
m17 = load_metric("i17")  # 海关进口（月频）

# i16 防城到港数据里有早期全 0 值，过滤出来只保留非零
d16_filtered = [[d, v] for d, v in pairs(m16) if v is not None and float(v) != 0]
d17 = pairs(m17)
d16_latest = max([d for d,_ in d16_filtered]) if d16_filtered else "-"

h1, j1 = chart_line_t("i16", "#7a8c5b", "万吨", "echart_61_c1",
    "铅精矿防城到港量（周频，非零数据）",
    "沸腾环贸 · 周 · 万吨 · %d点(非零) · 2020-01 至 %s" % (len(d16_filtered), d16_latest),
    default_seasonal=True, data=d16_filtered)

h2, j2 = chart_dual("echart_61_c2",
    "海关铅锭进口量 + 精矿防城到港量（成品 vs 原料双轨）",
    "海关(月,吨) · 防城(周,万吨) · %d / %d 点" % (m17["n"], len(d16_filtered)),
    d17, "#b06a32", "海关铅锭进口", "吨 (海关,月)",
...
<div class="header">
  ...
  <div class="hcrumbs">铅(PB) · 6 进出口 · 6.1 原料进口 · v1 2 图</div>
  <div class="hright">数据固化快照 · """ + NOW + """ · Zhiji SMM/Mysteel/海关</div>
</div>
<div class="panel">
""" + h1 + h2 + """
</div>
<footer>有色金属产业指标树 · 铅(PB) 6.1 原料进口 · v1（2 图全真数据）</footer>
...
window.addEventListener('resize',function(){['echart_61_c1','echart_61_c2'].forEach(function(id){var el=document.getElementById(id);var inst=echarts.getInstanceByDom(el);if(inst)inst.resize();});});
</script></body></html>"""
...
print("d16_filtered 点数:", len(d16_filtered), "d17:", len(d17))
...
## 近期变更记录
| 2026-09-13 | **[B-PB-CORRECTION] PB修正批次注册：27份correction文件→62新A级指标入库（v3.48→v3.49）** | Dsharnes-B | **工单**：PB_EXEC_TASK_20260913。**数据源**：translation-workspace/correction/ 下 CU/AL/NI/SN/ZN 28份修正 .md 文件 + cu-al-ni-sn/ 18份v3对照表（共46文件）。**Step2清洗**：解析3种表格格式（ZN旧版11列/AL新格式10列/cu-al-ni-sn v3 11-12列含✅A/⚠️B/❌C标记），自动提取zhiji_id+zhiji_name+node。**Step3知几匹配**：801条A级条目→去重后257条唯一zhiji_id→193条已注册/64条新注册（2条因ID重复跳过，实际注册62条）。**Step4差异比对**：三类差异已内含于修正表的A/B/C分级中，A级=知几验证通过可注册。**注册**：indicators_v1.json 930→992（+62条），v3.48→v3.49。分布：ZN 40条（供给3.1.x/库存4.x/进出口6.x/需求5.x）+ SN 2条 + AL 7条 + CU 1条 + NI 7条 + SN 5条。**门禁**：check_html 223/223 ✅ + verify_render 224/224 ALL PASS ✅。**注意**：62条指标为注册态（verified=true），api_cache.db 数据尚未拉取，需后续补拉后方可渲染正式页面。P2缺陷保留不阻断上线。|

...
| 日期 | 内容 |
|---|---|
| 2026-08-31 | [T14-AL2X] 铝2.x价格信号6子节点全上线(27→33页):①指标注册 indicators_v1.json v2.11→v3.4(162→179条)——7条 al_00_* 占位重定向到真实节点(2.1持仓/成交/收盘,2.2升贴水/基差,2.3 COMEX库存,2.6 COMEX CFTC净持仓占比) + 新增16条:2.2(al_22_spot现货价/al_22_premium_cn全国升贴水ID01319345)、2.3(LME 3M结算价FU00014308/持仓FU00014314/成交FU00014307)、2.4(LME月差锚+沪铝月差自算)、2.5(冶炼利润月ID01732414+2条估值分位自算)、2.6(沪铝08合约前20多/空单FU00021993/21998+COMEX非商业多空ID00303201/202)、2条月差衍生原料挂00;②三处自算并标注 _derived:2.4沪铝月差=主力结算价−连续结算价(2833点,Back981天/Contango1559天)、2.5估值分位=滚动730天窗口(沪铝87.1%/LME88.4%均处历史高位);③建页 al_2_1~al_2_6(2/2/3/2/2/3图=14图,零fetch);④al_2_overview 从规划占位改为6实链接(v1→v2),并修正其3处错误假设(占位页曾写「铝无COMEX」「2.1=进口盈亏」,实际知几有COMEX铝库存+席位、2.1按tree_config q=持仓/价/成交量);⑤引擎4项改进:MAIN_METRIC显式主图(4节点,防JSON排序/断更序列抢占正主)、--al-only/--cu-only按品种隔离(旧「按指标条数投票」在2.3/2.4/2.5混合节点误判,曾把铜cu_2_3/cu_2_4/cu_2_5覆盖成铝页)、is_stale断更保护(末点距今>180天剔除,al_22_spot实测n=1913但停在2022-10)、THEME_BY_COMM品种主题覆盖(铜2.x已验收口径锁定,al走tree_config)+code=CU显式传参沿用;⑥verify_render.js 的 ROOT 由硬编码改 path.resolve(__dirname,'..'),修复 git worktree 下 ENOENT。门禁:check_html 55/55 + verify_render 55/55 ALL PASS + reclaim PASS=12/FAIL=1(FAIL为历史[T14-*]前缀不在[A]/[B]/[DOC]白名单,非本次引入)。|
| 2026-08-30 | [T14-5.1] 铝5.1初级消费建页 + 引擎口径修复:①新增 al_51_util(SMM电解铝开工率:月度 a10146957,139点,2015-01..2026-07,11完整年)作正主,贴合tree_config q=开工率·同步(与铅5.1正主j51_util铅蓄电池开工率同构) + al_51_util_week(周度a12804269,86点)作高频辅助;②indicators_v1.json v2.10→v2.11(160→162指标);③正主防串用确认:al_52_cons(电解铝消费量月)已是5.2正主,5.1改用电解铝开工率不重复;④发现并记录 al_51_cons 语义错误——原挂5.1正主,但 zhiji_id ID01369383=氟化铝(含氟添加剂)表观消费,与电解铝无关,仅7个年点;保留不动以免破坏已建成的al_5_2.html补充图,在version_changelog留痕;⑤引擎修复3处:主图note「负值区=亏损」口径对百分比类指标错误(新增按unit分派:百分比走产能利用区间解读) + 落单辅助图note泛化(「结合主图判断…」)改为按freq分派(周度看边际/年度看确认) + 落单图补季节降级(原只对主图降级,辅助周频图残留季节按钮会产不出历年series);⑥重建9页(2.2/2.6/3.1.1/3.1.3/5.1/5.2/6.1/6.2/7.2,均走落单图路径受引擎改动影响);cu_2_1 手写样板页未重建(主脑已验收4图版,备份/tmp/cu_2_1.样板备份.html)。门禁:check_html 49/49 + verify_render 49/49 ALL PASS + reclaim 13/13。 |

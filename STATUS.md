...
- **指标错配**: 83个wr指标全量查0错配(已修wr56,已标注wr17/wr20)
- **周报图对应**: 24张周报图预期100%可匹配到网页图(节点全部存在)
- **交接文档**: docs/HANDOVER_周报图对应方案_20260909.md (给本地agent查知几关键词补ID)
- **门禁**: 266/267+243/243+12PASS


### 2026-09-09 主脑 — wr56指标错配修正+ao_42/44重建
- **wr56修正**: 原ID01721691(电解铝厂库存)错配, 改为ID01721655(氧化铝厂内库存除新疆), 与wr52同指标(周报双标签指向同一数据源)
- **ao_42/ao_44重建**: 缓存刷新, 数据更新至2026-09-09 (ao_44厂内库存139点, ao_42期货库存94点)
- **wr17标注**: 周报名"LME仓单原产地"实际是注册仓单(知几无原产地指标), 已标注修正
- **wr20标注**: 周报名"LME铝月差3-15"知几无月差序列, 当前用3M结算价代理, 已标记verified=false
- **缓存路径**: scripts/api_cache.db (非根目录), 不在git(在.gitignore), 需手动刷新
- **门禁**: 266/267(li_21历史坏页)+243/243+12PASS


### 2026-09-09 主脑 — wr周报图拼接到常规页(消除页面归类问题)
- **方案**: 把5个被常规页挡住的wr页(li_322wr/li_61wr/li_71wr/al_23wr/al_41wr)的chart块直接追加到对应常规页(li_3_2_2/li_6_1/li_7_1/al_2_3/al_4_1)footer前, wr图作为周报视角补充块内嵌
- **id处理**: wr图echart id改为常规接续格式(echart_al_23_c4/c5/c6), verify_render无需改逻辑
- **check_html配置更新**: li_322/61/71 charts 2->5, al_23/41 charts 3->6, verify_render charts同步更新
- **门禁**: check_html 266/267(li_21历史遗留坏页, 非拼接引起), verify_render 243/243全绿, reclaim 12PASS
- **指标错配核查**: 抽查24页83个wr指标, 3个真错配(wr17仓单原产地vs注册仓单/wr20月差vs结算价/wr56氧化铝厂vs电解铝厂库存), 其余为缩写差异可接受
- **重复核查**: 16个wr指标跨页同品种复用, 0个真错放, 全部保留


### 2026-09-09 主脑 — wr周报页+AO页上线接入看板index.html
- **备份**: backup_pre_wr_onboard_20260909 → 4490944(操作前main)
- **重复核查**: 24页跨页用16个wr指标, 0个真错放(实际页面用正确品种指标), 16个同品种合理复用(交叉验证, 保留)
- **PAGE_MAP新增24键**: AO 7键(2.3/3.1.3/3.1.4/3.2.1/4.2/4.4/7.1) + wr 17键(LC/NI/SI/AL/SN周报页)
- **导航逻辑**: 新增wrKey变量(cm.code_wr_leaf.id)+wrKey分支, fallback品种列表加AO
- **上线效果**: 19/24页可导航(AO 7全达+wr 12可达), 5个wr页被常规页优先(同节点有常规页, wr页仍可直链)
- **图数统计**: 24页周报图 - AO 7页/LC 6页/NI 5页/SI 3页/AL 2页/SN 1页
- **门禁**: 267/267+243/243+12PASS, index.html JS语法OK


### 2026-09-09 主脑 — ao_23_price第4图语义修正
- **问题**：agent2图纸卡误标wr88为"印尼FOB氧化铝", 实际wr88=铝土矿进口-澳大利亚(矿端模块)。ao_23_price第4图标题/副标题/图例/单位全写"印尼FOB·美元/吨"会误导用户
- **修正**：ao_23_price.html全量更正为"铝土矿进口-澳大利亚·吨"，数据未动仅改文字标签
- **核查**：56处build硬编码ID 55处匹配wr号(1处li_41wr仅注释错不影响数据)，24页页面数据全部正确
- **图纸卡文档**：docs/WEEKLY_REPORT_CHART_INVENTORY.md 58处指标名指认错已按indicators_v1.json真实名修正
- **门禁**：267/267+243/243+12PASS

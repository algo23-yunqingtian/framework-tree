...
| 2026-08-30 | **[T14-5.3] 铅需求·5.3需求先行指标子页上线 v1**（板块5第3子节点，全流程：Chrome CDP 同花顺发散→知几验证→注册→入库→build→push）。新增 `pb_53_demand_leading.html` 3图全真：图1 铅蓄电池下月预计开工率vs成品库存(月频双轴,5.3正主)、图2 下月预计开工率季节图(近5年历年线)、图3 再生铅原料库存天数vs废电瓶库存动态(月频双轴,原料端先行)。数据源=SMM下月预计开工率(a10151372 月90点/2017起)+SMM成品库存(i26/a12813406 月103点)+SMM再生铅天数(a12810628 月103点)+SMM废电瓶动态(a12767751 月103点)。indicators_v1.json v2.9→v3.0(+3指标 j53_next_rate/j53_waste_inv/j53_regen_days, 累计157指标)。发散记录 `analysis/iwencai/PB/53_diversify_20260830.md`。排除项：排产计划/订单/启停渗透率(知几无数据入备用库)；出口订单归6.3；再生铅产能利用率归3.2.3。**rebase冲突解**：indicators_v1.json 以 HEAD(154指标 j52_*)为基追加 j53_*→157指标；check_html/verify_render 取 ours(53注册)。三道门禁：check_html 18/18 ✅ + verify_render 18/18 ALL PASS ✅ + reclaim PASS=12 FAIL=1（预存 Windows 前缀 bug） | 线A |
